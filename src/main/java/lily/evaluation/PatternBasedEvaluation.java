...
public class PatternBasedEvaluation implements Evaluation {
    ...
    private double getScore(Position position) {
        return countPST(position) + position.getNumberOfWhite() * 1000 - position.getNumberOfBlack() * 1000 + position.getNumberOfWhiteKings() * 2500.0 - position.getNumberOfBlackKings() * 2500.0;
    }

    private TempScore cntPST(Position position) {
        NotationTranslator notationTranslator = new NotationTranslator();
        double scoreWhite = 0.0;
        double scoreBlack = 0.0;
        double scoreDraw = 0.0;
        ...
        for (int i = 0; i < patterns.length; i++) {
            ...
            for (int j = 0; j < pattern.length; j++) {
                if ((white & (1L << notationTranslator.toStandardField(pattern[j]))) > 0)
                    index += pow(3, pattern.length - j - 1);
                else if ((black & (1L << notationTranslator.toStandardField(pattern[j]))) > 0)
                    index += 2 * pow(3, pattern.length - j - 1);
            }
            index += i * 6561;

            scoreWhite += WHITE_WIN_WEIGHTS[index];
            scoreDraw += DRAW_WEIGHTS[index];
            scoreBlack += BLACK_WIN_WEIGHTS[index];
        }
        return new TempScore(scoreWhite, scoreDraw, scoreBlack);
    }

    private double countPST(Position position) {
        TempScore tempScore = cntPST(position);

        double scoreWhite = tempScore.whiteScore;
        double scoreBlack = tempScore.blackScore;
        double scoreDraw = tempScore.drawScore;

        int tempoDiff = position.getTempoDiff();
        int balanceDiff = position.getBalanceDiff();
        int menDiff = (position.getNumberOfWhite() - position.getNumberOfWhiteKings()) - (position.getNumberOfBlack() - position.getNumberOfBlackKings());
        int oneKingDiff = (position.getNumberOfWhiteKings() > 0 ? 1 : 0) - (position.getNumberOfBlackKings() > 0 ? 1 : 0);
        int multiKingDiff = Math.max(position.getNumberOfWhiteKings() - 1, 0) - Math.max(position.getNumberOfBlackKings() - 1, 0);

        scoreWhite += tempoDiff * WHITE_WIN_WEIGHTS[WHITE_WIN_WEIGHTS.length - 5];
        scoreDraw += tempoDiff * DRAW_WEIGHTS[DRAW_WEIGHTS.length - 5];
        scoreBlack += tempoDiff * BLACK_WIN_WEIGHTS[BLACK_WIN_WEIGHTS.length - 5];

        scoreWhite += balanceDiff * WHITE_WIN_WEIGHTS[WHITE_WIN_WEIGHTS.length - 4];
        scoreDraw += balanceDiff * DRAW_WEIGHTS[DRAW_WEIGHTS.length - 4];
        scoreBlack += balanceDiff * BLACK_WIN_WEIGHTS[BLACK_WIN_WEIGHTS.length - 4];

        scoreWhite += menDiff * WHITE_WIN_WEIGHTS[WHITE_WIN_WEIGHTS.length - 3];
        scoreDraw += menDiff * DRAW_WEIGHTS[DRAW_WEIGHTS.length - 3];
        scoreBlack += menDiff * BLACK_WIN_WEIGHTS[BLACK_WIN_WEIGHTS.length - 3];

        scoreWhite += oneKingDiff * WHITE_WIN_WEIGHTS[WHITE_WIN_WEIGHTS.length - 2];
        scoreDraw += oneKingDiff * DRAW_WEIGHTS[DRAW_WEIGHTS.length - 2];
        scoreBlack += oneKingDiff * BLACK_WIN_WEIGHTS[BLACK_WIN_WEIGHTS.length - 2];

        scoreWhite += multiKingDiff * WHITE_WIN_WEIGHTS[WHITE_WIN_WEIGHTS.length - 1];
        scoreDraw += multiKingDiff * DRAW_WEIGHTS[DRAW_WEIGHTS.length - 1];
        scoreBlack += multiKingDiff * BLACK_WIN_WEIGHTS[BLACK_WIN_WEIGHTS.length - 1];

        scoreWhite += WHITE_B0;
        scoreDraw += DRAW_B0;
        scoreBlack += BLACK_B0;
        scoreWhite = 1.0 / (1.0 + Math.exp(-scoreWhite));
        scoreDraw = 1.0 / (1.0 + Math.exp(-scoreDraw));
        scoreBlack = 1.0 / (1.0 + Math.exp(-scoreBlack));
        return (scoreWhite / (scoreWhite + scoreBlack + scoreDraw) - scoreBlack / (scoreWhite + scoreBlack + scoreDraw));
    }
    ...
    static class TempScore {
        double whiteScore;
        double drawScore;
        double blackScore;

        public TempScore(double whiteScore, double drawScore, double blackScore) {
